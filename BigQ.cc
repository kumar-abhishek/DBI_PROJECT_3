...
void * processBigQ(void *arg)
{
	...
	cout<<endl<< "filename: "<<fileNameStr<<endl;

	file.Open(0,fileNameStr);
	...
	while(1){
		...
		//sort vector 
		sort(recordVector.begin(),recordVector.end(),compareRecords(sortOrder));
		Record *l, *r;
	...
	//priority_queue<pair<Record *,int>,vector<pair<Record *,int> >, recordComparisonForPriorityQueue >pqRecords(recordComparisonForPriorityQueue(sortOrder));
	typedef priority_queue<pair<Record *,int>,vector<pair<Record *,int> >, recordComparisonForPriorityQueue > recPq;
	recPq pqRecords(sortOrder);
